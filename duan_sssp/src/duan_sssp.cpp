...
// Global statistics
// WARNING: g_stats and g_collect_stats are NOT thread-safe.
// Do not use collect_stats=true in multi-threaded contexts, or protect
// access with external synchronization. Each thread should use its own
// DuanSSSP instance with collect_stats=false for thread-safe operation.
DuanStats g_stats;
bool g_collect_stats = false;

DuanSSSPResult DuanSSSP::ComputeSSSP(
    const Graph& graph,
    int source,
    bool collect_stats) {
...
int DuanSSSP::ComputeInitialLayer(int n, const Params& params) {
    ...
    // Handle edge case where params.t is 0 to avoid division by zero
