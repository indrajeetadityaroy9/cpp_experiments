...
BMSSPResult BMSSP::Execute(
    ...
    // Base case: layer 0
    if (l == 0) {
        auto base_result_exp = BaseCase::Execute(graph, labels, B, S, params.k);
        if (!base_result_exp) {
            // Error in BaseCase - return empty result with boundary B
            current_recursion_depth--;
            return BMSSPResult{B, {}};
        }
    ...
    // M = 2^((l-1)t) with overflow protection
    ...
    // k_limit = k * 2^(lt) with overflow protection
