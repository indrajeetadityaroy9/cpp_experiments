...
struct BaseCaseResult {
    long double b;  // Boundary value: max distance of returned vertices, or B if fewer than k+1 found
    vector<int> U;  // Completed vertices with distance < b
};
...
public:
     ...
     * @return BaseCaseResult on success, DuanError on failure
     */
    [[nodiscard]] static std::expected<BaseCaseResult, DuanError> Execute(
        const Graph& graph,
        Labels& labels,
        long double B,
        const vector<int>& S,
