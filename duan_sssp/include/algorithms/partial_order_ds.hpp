...
private:
    ...
    /**
     * Update key location tracking for O(1) element access
     */
    ...
    /**
     * Remove key from location tracking on element deletion
     */
    ...
    /**
     * Rebuild D1 bounds BST after block operations invalidate iterators
     * Time: O(N/M) where N is total elements
     */
    void RebuildD1Bounds();

    /**
     * Rebuild key location tracking after block operations
     * Required after erase() which invalidates stored iterators
     * Time: O(N)
     */
    void RebuildKeyLocations();

    /**
     * Find minimum value across remaining elements in D0 and D1
     * Returns INF if data structure is empty
     * Time: O(1) - only checks front elements
     */
