    ...
    void build(int n, const vector<tuple<int, int, long double>>& edges, int s_orig) {
        ...
        for (int v = 0; v < n; ++v) {
            ...
            for (size_t i = 0; i < pairs.size(); i++) {
                Gt[pairs[i].second].push_back({pairs[(i + 1) % pairs.size()].second, 0.0L});
            }
        }
        ...
        for (const auto& e : edges) {
            ...
            long double w;
            tie(u, v, w) = e;
            Gt[slot_id[u].at(v)].push_back({slot_id[v].at(u), w});
        }
    }
};
...
struct PartialOrderDS {
    ...
    void Insert(int key, long double val) {
        if (pos[key].block_idx != -1) {
            ...
            if (val >= items[pos[key].item_idx].val) {
                return;
            }
        }
        ...
        if ((int)target_blk.items.size() > M) {
            ...
            std::nth_element(items.begin(), median_it, items.end(), [](const Entry& a, const Entry& b) {
                return a.val < b.val;
            });
...
struct DMSY_SSSP {
    ...
    DMSY_SSSP(const Graph& Gt, int s_) : G(Gt), N((int)Gt.size()), s(s_) {
    ...
    ~DMSY_SSSP() {
        ...
        for (int i = 0; i < NUM_KBATCH_LOCKS; ++i) {
            omp_destroy_lock(&Kbatch_locks[i]);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if ((long double)m < m_crit) {
        cout << "Regime: Sparse. Paper's algorithm is expected to be asymptotically faster." << endl;
    } else {
        cout << "Regime: Dense. Dijkstra's algorithm is expected to be asymptotically faster." << endl;
    }
